...
uri="http://java.sun.com/jsp/jstl/core" %> <% String
root=request.getContextPath(); // delete ".jsp" String
URI=request.getRequestURI(); URI=URI.substring(0, URI.length() - 4); %>
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container px-20 py-20">
      <table
        class="w-full text-sm text-left rtl:text-right text-gray-700 text-gray-600"
      >
          ...
          <tbody>
            ...
            <c:forEach var="ticket" items="${invoice.tickets}">
              <tr class="border-b bg-gray-100 border-gray-200">
                <c:choose>
                  <c:when
                    test="${ticket.invoice.getInvoiceID() != currentInvoiceId}"
                  >
                  ...
                  <c:otherwise>
                    <th scope="row" class="px-6 py-2"></th>
                    <!-- <th scope="row" class="px-6 py-2 font-medium text-gray-700">
                                                    ${ticket.getTicketID()}
                                                </th> -->
                    <th
